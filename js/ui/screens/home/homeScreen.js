...
export const HomeScreen = {
  ...
  ensureMainVerticalVisibility(target) {
    ...
    // PERF: offsetTop does not force layout reflow; getBoundingClientRect() does
    const pad = 14;
    ...
    if (targetBottom > viewBottom - pad) {
      main.scrollTop = targetBottom - main.clientHeight + pad;
    } else if (targetTop < viewTop + pad) {
      main.scrollTop = targetTop - pad;
    }
  },
  ...
  ensureTrackHorizontalVisibility(target, direction = null) {
    ...
    // PERF: use offsetLeft (no reflow) instead of getBoundingClientRect
  ...
  focusNode(current, target, direction = null) {
    ...
    // PERF: cache focused node to avoid querySelectorAll on next keypress
  ...
  handleHomeDpad(event) {
    ...
    const current = this._focusedNode || this.container?.querySelector(".focusable.focused");
    if (!current) {
      return false;
    }
        ...
        <main class="home-main">
          <section class="home-hero">
            <div class="home-hero-card focusable"
              ...
              ${heroItem?.type ? `<div class="home-hero-meta"><span class="home-hero-badge">${heroItem.type === "series" ? "Series" : "Film"}</span>${heroItem?.year ? `<span class="home-hero-year">${heroItem.year}</span>` : ""}</div>` : ""}
              ...
              <div class="home-hero-actions">
                <button class="home-hero-btn home-hero-btn-play" tabindex="-1" aria-hidden="true">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
                  Play
                </button>
                <button class="home-hero-btn home-hero-btn-info" tabindex="-1" aria-hidden="true">
    ...
    if (rowsContainer) {
      ...
      this.rows.forEach((rowData) => {
        ...
        rowData.result.data.items.slice(0, this.layoutMode === "grid" ? 12 : 16).forEach((item) => {
          ...
          // PERF: loading="lazy" defers off-screen image fetching
          card.innerHTML = `
            ${item.poster ? `<img class="content-poster" src="${item.poster}" alt="${item.name || "content"}" loading="lazy" decoding="async" />` : `<div class="content-poster placeholder"></div>`}
            <div class="home-content-title">${item.name || ""}</div>
          `;
